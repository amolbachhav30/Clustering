...
proptemp# Compute the number of observations per group
...
library("tidyverse")
library("dplyr")
first_clust <- aggdata%>%filter(aggdata$groups==1)
second_clust<-aggdata%>%filter(aggdata$groups==2) 
second_clust
...
library(ggplot2)
ggplot(data, aes(LPE,NP))+ geom_point()
```
